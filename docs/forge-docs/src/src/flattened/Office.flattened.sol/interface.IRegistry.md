...
**Inherits:**
[IERC6909TokenSupply](/src/flattened/Office.flattened.sol/interface.IERC6909TokenSupply.md)
...
*Note that this function only returns allowance as set by the current owner of the
`owner` user account.*
...
*The allowances DO NOT carry over once the account is transferred to a new owner.*
...
*A collateral token ID is technically a token ID but additional details like
the token type (e.g. escrow or share).*

*Note that this function is gas-intensive and this limits the amount of collateral assets
...
*NOTE: The following could deviate from the ERC6909 standard given that an operator cannot transfer
...
```solidity
event Registry__Approval(AccountId indexed account, AccountId indexed spender, uint256 indexed tokenId, uint256 amount);
```
...
```solidity
event Registry__OperatorSet(address indexed owner, AccountId indexed account, address indexed operator, bool approved);
```
