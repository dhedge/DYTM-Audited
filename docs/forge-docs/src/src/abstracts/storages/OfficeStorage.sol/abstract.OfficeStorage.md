# OfficeStorage
[Git Source](https://github.com/dhedge/DYTM/blob/7e26fc0ea2ef08e01be7caaabb0d58657fa9304e/src/abstracts/storages/OfficeStorage.sol)
...
Abstract contract that manages the storage for the Office contract.
...
|Name|Type|Description|
|----|----|-----------|
|`marketCount`|`uint88`|The total number of markets created in the Office.|
|`officers`|`mapping(MarketId market => address officer)`|Mapping of market ID to officer address.|
|`reserveData`|`mapping(ReserveKey key => ReserveData data)`|Mapping of reserve key to reserve data.|
|`configs`|`mapping(MarketId market => IMarketConfig config)`|Mapping of market ID to market configuration contract.|
